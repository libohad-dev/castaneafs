...
9. **Symlink traversal and security conflict scope**: If a file is reachable through a symlink, and a concurrent transaction relaxes permissions on a directory in the symlink's path (not the target's ancestor chain), does that trigger a security conflict with writes to the target? Rule 2 covers ancestor directories of the written file, but the symlink's directory chain is a separate resolution path to the same file. The relaxation makes the file discoverable through the symlink path even if the target's own ancestor permissions are unchanged.

10. **Transaction view mount lifecycle**: When a transaction reaches a terminal state (committed, aborted, or failed→aborted) while a transaction view mount is active, what happens to the mount? Possible behaviors include: the mount auto-unmounts, the mount becomes an error filesystem (all operations return an error), or the mount persists in a read-only state (for failed→aborted, showing the last failed state). The interaction with in-flight operations on the mount at the moment of state transition also needs to be defined.

11. **Caller identity for non-FD operations**: For operations that do not go through `open()` (e.g., `mkdir`, `unlink`, `chmod`, `stat`), the implicit subtransaction's commit-time permission check (commit rule 3) requires a UID/GID. The FUSE request context provides the caller's `uid` and `gid` for each request. The spec should confirm that for non-FD operations, the identity from the FUSE request context is used for commit-time permission evaluation, paralleling how FD-based operations use the access mode established at `open()` time.
...
- **2026-03-03**: Added open questions: symlink traversal and security conflict scope (#9), transaction view mount lifecycle after terminal state (#10), caller identity for non-FD operations (#11).
