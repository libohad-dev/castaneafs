...
**Content domain.** File data operations (reads, writes, truncates) and directory entry operations (create, delete, rename). Standard first-committer-wins applies within this domain: if two sibling transactions both modify the content domain of the same inode, the second to commit fails.
...
- **Permission bits (mode).** When two sibling transactions both modify the same inode's permission bits, the server compares each transaction's end state against its snapshot start. If both are *pure expansions* — each end state is a bitwise superset of its snapshot start (`end | snapshot == end`, meaning the transaction only added bits, never removed any) — the changes are non-conflicting. The merged result is the bitwise OR of both end states (commutative, associative, order-independent). If either transaction removed any bit from its snapshot state, standard first-committer-wins applies. Rationale: a relative `chmod g+r` translates to an absolute mode where the group-read bit is added and all other bits are preserved — a pure expansion. An absolute `chmod 640` that removes bits the snapshot had is expressing intent to restrict, which may contradict a concurrent expansion. The server cannot distinguish "didn't care about this bit" from "deliberately removed this bit" since both produce the same absolute end state.

- **Ownership (uid, gid).** Ownership is a discrete value, not a bitmask; no natural merge operation exists. Convergent changes (identical final uid and gid) produce no conflict. Divergent changes use first-committer-wins.
...
**Pairwise sufficiency of security checks.** When multiple permission-changing transactions merge via the pure-expansion rule (bitwise OR), the merged result cannot exceed the writing transaction's reference if no individual transaction does. If each transaction's security-relevant bits are a subset of the reference, their union is also a subset (monotonicity of bitwise OR over the subset relation). Pairwise security checks against each permission-changing transaction are therefore sufficient.
