...
10. **Caller identity for non-FD operations**: For operations that do not go through `open()` (e.g., `mkdir`, `unlink`, `chmod`, `stat`), the implicit subtransaction's commit-time permission check (commit rule 3) requires a UID/GID. The FUSE request context provides the caller's `uid` and `gid` for each request. The spec should confirm that for non-FD operations, the identity from the FUSE request context is used for commit-time permission evaluation, paralleling how FD-based operations use the access mode established at `open()` time.
