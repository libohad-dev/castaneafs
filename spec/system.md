...
2. **File attribute (`stx_attributes`) conflict domain and merge strategy**: File attributes are a bitmask like permission bits, but their semantics differ fundamentally. Permission bit expansion (adding `+r`) increases access; file attribute changes are mixed — setting `FS_IMMUTABLE_FL` *restricts* operations (prevents modification), while clearing it *relaxes* them. The "pure expansion via bitwise OR" reconciliation used for permission bits assumes that OR-merging is safe because it only widens access; this reasoning does not transfer to file attributes where setting a bit can be restrictive.

   Key design choices: (a) Should file attributes form a **separate conflict domain** from permission metadata? Permission bits control *who* can access; file attributes control *what operations* are possible on the inode. Combining them means a `chmod` and a `chattr` on the same inode always conflict within the same domain, which may be unnecessarily conservative. Separating them allows independent modification but requires defining cross-domain interaction rules. (b) Is there a meaningful **merge operation** for concurrent file attribute changes, or should the domain use first-committer-wins exclusively? Unlike permission bits, where the subset lattice provides a natural merge direction, file attributes have no uniform lattice — "more restrictive" and "more permissive" depend on the individual flag. A per-flag analysis (e.g., immutable is restrictive when set, permissive when cleared) could enable selective reconciliation, but the flag-level complexity may not justify the concurrency benefit over simple first-committer-wins.

3. **File attribute interaction with security conflict rules and POSIX checks**: Clearing restrictive file attributes (immutable, append-only) is analogous to permission relaxation — it enables operations that were previously forbidden. If one transaction clears `FS_IMMUTABLE_FL` and another writes content to the same file, the writer may have assumed the file was protected against modification by other processes. This parallels the reasoning behind Rules 1–2 (content write + permission relaxation). Conversely, setting immutable or append-only is analogous to permission tightening, which is established as a non-conflict.

   However, file attributes differ from permissions in a key respect: they do not affect *who can read* the file (the confidentiality concern motivating Rules 1–2), but rather *what operations are possible* (an integrity/mutability concern). Rules 1–2 address data exposure; file attribute changes address data modifiability. This raises the question of whether the existing rules extend (treating "clearing immutable" as "more permissive" in an expanded definition) or whether a separate rule is needed that addresses the distinct threat model.

   For POSIX-level checks at commit time: if one transaction sets `FS_IMMUTABLE_FL` and commits, a concurrent write should fail — the file is now immutable. File attributes are checked by the VFS layer (`IS_IMMUTABLE(inode)`) separately from standard permission checks. The existing commit rule 3 (POSIX permission check) would need to be extended to include file attribute enforcement, or a new commit rule condition would be added.

4. **Extended attribute conflict granularity and merge strategy**: Extended attributes are keyed name-value pairs with opaque byte-sequence values. Unlike permission bits (algebraic merge via OR) or ownership (convergence check), xattr values have no general algebraic structure — no natural merge operation exists for arbitrary byte sequences. The design must choose a conflict detection granularity and determine whether namespace-level distinctions affect the conflict model.
