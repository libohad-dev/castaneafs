...
The bitmap merge is structurally a **state-based CRDT** (conflict-free replicated data type): the change-mask accumulation operates via bitwise OR, which is commutative, associative, idempotent, and monotone over the set-inclusion lattice (the change-mask set is grow-only). These algebraic properties guarantee that the merged bitmap value is independent of the order in which transactions commit — given a snapshot `S` and any number of concurrent committed transactions with change masks `Δᵢ = S ⊕ Eᵢ`, the final state is `S ⊕ (Δ₁ | Δ₂ | … | Δₙ)` regardless of commit sequence. The same merge formula applies to file attribute flags (`stx_attributes`) when that feature is added, since file attributes are also a bitmap set via absolute-value ioctls (`FS_IOC_SETFLAGS`). This current-state comparison approach — validating the committing transaction against the parent's committed state rather than enumerating individual concurrent transactions — follows the same pattern used by optimistic concurrency control systems generally: FoundationDB's Resolver checks a transaction's read set against a unified map of recent committed writes; CockroachDB's read-refresh validates against the MVCC version history in a timestamp range; PostgreSQL's snapshot isolation checks a tuple's `xmax` against the transaction's snapshot. In all cases, the current state subsumes the chain of intermediate commits. CastaneaFS's contribution is applying unconditional reconciliation within this model, using the algebraic properties of the XOR/OR merge to guarantee that the reconciled result is deterministic and order-independent without requiring knowledge of the individual transactions that produced the current state.
