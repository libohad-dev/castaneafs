...
> **Advisory note — FUSE request splitting.** Several operations that appear atomic to userspace are split into multiple independent FUSE requests by the kernel before reaching the daemon. The resulting behavior is not atomic — concurrent operations may interleave between requests. The daemon-side implicit transaction wrapping does not help, since the splitting occurs in the kernel before the requests arrive. For proper isolation from concurrent processes, these operations should be explicitly guarded by a dedicated transaction.
>
> - **Large reads and writes.** The FUSE protocol splits read and write operations into chunks bounded by the `max_read` and `max_write` values negotiated during `FUSE_INIT`. A single userspace `read()` or `write()` call may arrive at the daemon as multiple FUSE requests, with concurrent operations on the same file potentially interleaving between chunks. This is consistent with conventional filesystem behavior (POSIX does not guarantee atomicity for large I/O operations).
> - **Directory listing.** `FUSE_READDIR` responses are bounded by a size-limited buffer. If a directory has more entries than fit in one response, the kernel issues multiple READDIR requests with increasing offsets. A userspace `readdir()` loop may therefore observe an inconsistent snapshot — entries added or removed by concurrent operations between requests.
> - **Memory-mapped I/O.** Writes to memory-mapped regions are flushed as individual page-sized WRITE requests (typically 4 KiB), triggered by `msync()`, memory pressure, or periodic kernel writeback. A process modifying a large memory-mapped region generates many independent FUSE writes at page granularity, potentially interleaved with concurrent operations. This non-atomicity is well-understood in conventional filesystems, but in CastaneaFS's context each page write is its own implicit subtransaction.
...
- `STATX_SUBVOL`: Not supported. The `stx_subvol` field (added in Linux 6.10 for btrfs and bcachefs) exposes a subvolume identifier, extending file identity from `(st_dev, st_ino)` to `(st_dev, stx_subvol, st_ino)`. CastaneaFS transactions are conceptually analogous to subvolumes, but `stx_subvol` is not a viable identity mechanism for several reasons: the FUSE wire protocol's `struct fuse_statx` has no `subvol` field (so FUSE daemons cannot report it regardless of intent), no major userspace tools (coreutils, findutils, rsync, tar) consume it, and the kernel community does not consider it a universal solution — it only helps filesystems with subvolumes and requires applications to have filesystem-specific knowledge, violating the VFS abstraction. The kernel community's long-term direction for file identity is file handles (`name_to_handle_at(2)` / `open_by_handle_at(2)`), which are opaque, variable-length, filesystem-agnostic, and already used by NFS and fanotify. If CastaneaFS needs to expose transaction identity through `statx` in the future, it would depend on FUSE protocol extensions.
- Other `STATX_*` fields (e.g., `STATX_DIOALIGN`): Not supported; corresponding mask bits are unset in the response.
...
- **2026-03-11**: Expanded `STATX_SUBVOL` documentation in the `statx` behavior section. Documented why it is not a viable identity mechanism for CastaneaFS (FUSE wire protocol lacks the field, no userspace tool adoption, not considered a universal solution by the kernel community — requires filesystem-specific knowledge, violating the VFS abstraction). Noted file handles (`name_to_handle_at`/`open_by_handle_at`) as the kernel community's long-term direction for file identity.
- **2026-03-15**: Added advisory note on FUSE request splitting: three categories of operations that appear atomic to userspace are split into multiple independent FUSE requests by the kernel — large reads/writes (chunked by `max_read`/`max_write`), directory listing (size-limited READDIR responses), and memory-mapped I/O (page-granularity writeback). Each request gets its own implicit subtransaction with no cross-request atomicity. Users requiring isolation should use explicit transactions.
